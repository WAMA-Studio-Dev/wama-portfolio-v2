...
interface FloatingLinesProps {
  /** Hasta 8 colores hex; sustituye al degradado rosa/azul por defecto del shader. */
  ...
  mixBlendMode?: string;
  /** Extra para integrarlo en WAMA (p.ej. `absolute inset-0` + máscara de
   * desvanecimiento), aplicado sobre el mismo contenedor que ya lleva el
   * mix-blend-mode — no añade ningún wrapper nuevo. */
  className?: string;
}

/**
 * Adaptación TypeScript del componente FloatingLines de React Bits. La
 * lógica del shader/animación/three.js es la del original, sin modificar;
 * solo se ha tipado y se ha añadido `className` para integrarlo sin un
 * wrapper extra (ver comentario junto al render).
 */
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const setupScene = () => {
      ...
      // Evita el flush síncrono que WebGLRenderer hace por defecto para
      // comprobar errores de compilación del shader en cada uso del programa.
      ...
      // Igual que el resto de listeners globales de "pointermove" del sitio:
      // se agrupa a un máximo de una vez por frame (rAF) en vez de procesar
      // cada evento nativo, que puede llegar muy por encima de 60Hz y forzaba
      // una lectura de layout (getBoundingClientRect) en cada uno.
